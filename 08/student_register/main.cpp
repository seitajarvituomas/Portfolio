...
void change_number(std::string student_number, std::map< std::string, Student* >& student_numbers) {
    ...
    if (it != student_numbers.end()) {
        ...
        if (is_valid_phone_number(new_phone_number)) {
            it -> second -> phone_number = new_phone_number;
...
int main() {
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            change_number(parts.at(1), student_numbers);

            std::ofstream output(file_name);

            for (const auto& pair : user_ids) {
                const Student* student = pair.second;
                output << student->student_number << ";"
                       << student->user_id << ";"
                       << student->name << ";"
