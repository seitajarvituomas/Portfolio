...
string to_uppercase( const string& feed) {
    string result = feed;
...
int main() {
    ...
    while ( true ) {
        ...
        command_to_be_executed = to_uppercase(command_to_be_executed);
        ...
        for (const Command& command : COMMANDS) {
            if (command.str == command_to_be_executed) {
                 ...
                 if (command.parameter_number +1 == pieces.size()) {
                     ...
                     if (string_to_double(pieces.at(1), left) && string_to_double(pieces.at(2), right)) {
                         double result = command.action(left,right);
                         cout << result << endl;
                         command_found = true;
                         break;
